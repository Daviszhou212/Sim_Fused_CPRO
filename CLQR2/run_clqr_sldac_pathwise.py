...
# 固定复现实验组：与当前 CLQR2 SLDAC 保持同一组 run_tag。
SLDAC_PATHWISE_RUNS = [
    ("b100_q5", "SLDAC_Pathwise, T=500, batchsize=100, q=5", 250, 250, 100, 5),
]

# 顶部集中配置：除非用户显式要求，否则以这里为准。
DEFAULT_SEED = 1
DEFAULT_SEEDS = (40, )
...
def _run_single_seed(args):
    for run_tag, message, t_horizon, grad_t, num_new_data, q_update_time in SLDAC_PATHWISE_RUNS:
        print(message)
