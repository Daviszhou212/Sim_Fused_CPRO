...
class QPropCriticTest(unittest.TestCase):
    def test_clqr_head_count_and_invalid_constraint_dim(self):
        critic = QPropCritic("CLQR", 15, 4, 1, "cpu")
        self.assertEqual(critic.head_count, 2)
        with self.assertRaises(ValueError):
            QPropCritic("CLQR", 15, 4, 4, "cpu")
    ...
    def test_update_from_replay_returns_finite_diagnostics_and_detaches_actor(self):
        ...
        critic = QPropCritic("CLQR", state_dim, action_dim, constraint_dim, "cpu")
    ...
    def test_target_head_value_keeps_action_gradient(self):
        critic = QPropCritic("CLQR", 15, 4, 1, "cpu")
        state = torch.randn(6, 15, dtype=torch.float)
        action = torch.randn(6, 4, dtype=torch.float, requires_grad=True)
