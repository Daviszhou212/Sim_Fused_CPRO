	...
	for t in range(MAX_STEPS):
		...
		if t > 2*T and ((t-2*T) % (num_new_data/Q_update_time) == 0):
			...
			if (update_index % update_time_per_episode == 0) and (Q_update_index == 1):
				...
				print('reward_average: ', np.mean(aver_reward_buffer))
				print('cost_average: ', np.mean(aver_cost_buffer))
				reward_average_save.append(np.mean(aver_reward_buffer))
				cost_average_save.append(np.mean(aver_cost_buffer))
				print_index += 1
