...
from run_mimo_fused_cpro import build_python_config as build_fused_cpro_python_config
from run_mimo_hrl import build_python_config as build_hrl_python_config
from run_mimo_hrl import _resolve_new_policy_init_args as resolve_hrl_new_policy_init_args
...
class NewPolicyInitCheckpointTest(unittest.TestCase):
    ...
    def test_new_actor_init_skips_missing_log_std_when_disabled(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            checkpoint_dir = os.path.join(tmpdir, "MIMO", "b100_q10", "seed_{0}".format(7))
            os.makedirs(checkpoint_dir, exist_ok=True)
    ...
    def test_old_policy_checkpoint_loads_mu_only_and_keeps_default_log_std(self):
        _, source_actor, state_dim, action_dim, constraint_dim, _ = _build_scene("MIMO", 7, "cpu", 1)
        _, default_actor, _, _, _, _ = _build_scene("MIMO", 0, "cpu", 1)
