	...
	for t in range(MAX_STEPS):
		...
		if t > 2*T and ((t-2*T) % (num_new_data/Q_update_time) == 0):
			...
			if (update_index % update_time_per_episode == 0) and (Q_update_index == 1):
				should_print = (print_interval > 0) and (
					(print_index % print_interval == 0) or (print_index == getattr(args, 'episode', -1) - 1)
				)
				if should_print:
					print('SCAOPO_EPISODE: ', print_index, flush=True)
					print('reward_average: ', np.mean(aver_reward_batch), flush=True)
					print('cost_average: ', np.mean(aver_cost_batch), flush=True)
