...
def _create_actor(state_dim, action_dim, device, batch_size, init_log_std):
...
def _rollout_batch(env, actor, normalizer, horizon, constraint_limit, device, observation):
    states = []
    norm_states = []
    actions = []
    objective_costs = []
    ...
    for _ in range(int(horizon)):
        ...
        cost_residuals.append(raw_cost - float(constraint_limit))
    ...
    return {
        ...
        "cost": np.asarray(raw_costs, dtype=np.float64),
...
def ACPO_main(args, example_name):
    ...
    constraint_limit = float(args.constraint_limit)
