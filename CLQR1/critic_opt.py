...
from model import Critic_net_CLQR_0
from model import Critic_net_CLQR_1
...
class Critic:
    ...
    def __init__(self, example_name, num_new_data, state_dim, action_dim, constraint_dim, q, device):
        ...
        if "MIMO" in self.example_name:
            ...
            # ----------------------
            self.net0 = Critic_net_CLQR_0(self.state_dim, self.action_dim, self.device)
            self.target_net0 = Critic_net_CLQR_0(self.state_dim, self.action_dim, self.device)
            self.critic0_optimizer = torch.optim.Adam(self.net0.parameters(), 0.1/self.q**0.7)
            hard_update(self.target_net0, self.net0)
            self.net1 = Critic_net_CLQR_1(self.state_dim, self.action_dim, self.device)
            self.target_net1 = Critic_net_CLQR_1(self.state_dim, self.action_dim, self.device)
            self.critic1_optimizer = torch.optim.Adam(self.net1.parameters(), 0.005/self.q**0.7)
            hard_update(self.target_net1, self.net1)
