...
def SCAOPO_main(args, example_name):
    ...
    if 'MIMO' in example_name:
        ...
        constr_lim = np.array([1.2, 1.2, 1.2, 1.2])
        actor = GaussianPolicy_MIMO(state_dim, action_dim, device, temp)
    else:
        ...
        actor = GaussianPolicy_CLQR(state_dim, action_dim, device, temp)
    ...
    for t in range(MAX_STEPS):
        ...
        # update the policy
        if t >= 2*T and ((t-2*T) % num_new_data == 0):
            update_index += 1
            alpha = 1 / (update_index ** alpha_pow)
            beta = 1 / (update_index ** beta_pow)
            state_batch, action_batch, costs_batch, next_state_batch, aver_reward_batch, aver_cost_batch = buffer.take_experiences()
            ...
            if (update_index % update_time_per_episode == 1):
                ...
                print('cost_average: ', np.mean(aver_cost_batch))
                reward_average_save.append(np.mean(aver_reward_batch))
                cost_average_save.append(np.mean(aver_cost_batch))
                print_index += 1
