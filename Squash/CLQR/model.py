...
class Critic_net_CLQR_0(nn.Module):
	...
	def __init__(self, state_dim, action_dim, device):
		...
		:return:
		"""
		super(Critic_net_CLQR_0, self).__init__()
...
class Critic_net_CLQR_1(nn.Module):
	...
	def __init__(self, state_dim, action_dim, device):
		...
		:return:
		"""
		super(Critic_net_CLQR_1, self).__init__()
...
class GaussianPolicy_MIMO(nn.Module):
	...
	def __init__(self, state_dim, action_dim, device, num_new_data):
		...
		self.fc1_dim = 128
		self.fc2_dim = 128
		self.net = MLP_Gaussian_MIMO(state_dim, self.fc1_dim, self.fc2_dim, action_dim, device)
