...
def _sample_hybrid_action(state, actor_new, old_policies, rho):
    rho_arr = np.asarray(rho, dtype=np.float64).reshape(-1)
    choice = int(np.random.choice(rho_arr.size, p=rho_arr))
    if choice == 0:
        return _sample_action(actor_new, state)
    return np.asarray(old_policies[choice - 1].sample_action(state), dtype=np.float64).reshape(-1)


def _sample_hybrid_action_batch(state_batch, actor_new, old_policies, rho, action_dim):
    states = np.asarray(state_batch, dtype=np.float64)
    if states.ndim != 2 or states.shape[0] <= 0:
        return np.zeros((0, action_dim), dtype=np.float64)
    actions = np.zeros((states.shape[0], action_dim), dtype=np.float64)
    for idx in range(states.shape[0]):
        actions[idx] = _sample_hybrid_action(states[idx], actor_new, old_policies, rho)
    return actions
...
def _policy_rollout_dataset(example_name, policy, steps, seed, device):
    ...
    next_states = np.zeros((steps, state_dim), dtype=np.float64)
    for idx in range(int(steps)):
        state = observation
        action = _sample_action(policy, state)
        ...
        next_states[idx] = np.asarray(next_state, dtype=np.float64).reshape(-1)
        observation = next_state
        if done:
            observation = env.reset()
    return {"state": states, "action": actions, "costs": costs, "next_state": next_states}
...
def _sample_offline_transition_batch(datasets, batch_size, state_dim, action_dim, cost_dim, dataset_probs=None):
    batch_size = int(batch_size)
    if batch_size <= 0 or len(datasets) <= 0:
        return (
            np.zeros((0, state_dim), dtype=np.float64),
            np.zeros((0, action_dim), dtype=np.float64),
            np.zeros((0, cost_dim), dtype=np.float64),
            np.zeros((0, state_dim), dtype=np.float64),
        )
    dataset_probs = _coerce_dataset_sampling_probs(len(datasets), dataset_probs)
    states = np.zeros((batch_size, state_dim), dtype=np.float64)
    ...
    next_states = np.zeros((batch_size, state_dim), dtype=np.float64)
    for idx in range(batch_size):
        ds = datasets[int(np.random.choice(len(datasets), p=dataset_probs))]
        row = np.random.randint(ds["state"].shape[0])
        states[idx] = ds["state"][row]
        ...
        next_states[idx] = ds["next_state"][row]
...
def _pack_xi_history(actor_xi_values, critic_xi_values):
    return {
        "actor": np.asarray(actor_xi_values, dtype=np.float64).reshape(-1),
        "critic": np.asarray(critic_xi_values, dtype=np.float64).reshape(-1),
    }


def _use_legacy_critic_mode(use_offline_data, critic_xi):
    return (not bool(use_offline_data)) or (float(critic_xi) <= EPS)
    ...
    for t in range(max_steps):
        ...
        if t > 2 * t_horizon and ((t - 2 * t_horizon) % (num_new_data / q_update_time) == 0):
            ...
            if q_update_index == q_update_time:
                ...
                q_hat_torch = critic.critic_value(state_batch_torch, action_batch_torch, legacy_online_mode=True)
...
def _run_policy_mix_main(args, example_name, algorithm_label, use_offline_data):
    ...
    actor_xi0 = float(getattr(args, "actor_xi0", legacy_xi0))
    actor_xi_pow = float(getattr(args, "actor_xi_pow", legacy_xi_pow))
    critic_xi0 = float(getattr(args, "critic_xi0", legacy_xi0))
    critic_xi_pow = float(getattr(args, "critic_xi_pow", legacy_xi_pow))
    for name, value in (("actor_xi0", actor_xi0), ("critic_xi0", critic_xi0)):
        if (value < 0.0) or (value > 1.0):
            raise ValueError("{0} must be in [0, 1] as offline weight. got {0}={1}".format(name, value))
    for name, value in (("actor_xi_pow", actor_xi_pow), ("critic_xi_pow", critic_xi_pow)):
        if value <= 0.0:
    ...
    cost_average_save = []
    rho_history_save = []
    actor_xi_history_save = []
    critic_xi_history_save = []
    ...
    for t in range(max_steps):
        state = observation
        action = _sample_hybrid_action(state, actor_new, old_policies, rho)
        observation, reward, done, info = env.step(action)
        next_state = observation
        costs = _build_costs(reward, info, constraint_dim, constr_lim)
        aver_cost = float(info.get("cost", 0.0)) / max(constraint_dim, 1)
        ...
        if t > 2 * t_horizon and ((t - 2 * t_horizon) % (num_new_data / q_update_time) == 0):
            ...
            if (update_index % update_time_per_episode == 0) and (q_update_index == 1):
                ...
                if use_offline_data:
                    ...
                    print("actor_xi_offline_weight:", float(actor_xi))
                    print("critic_xi_offline_weight:", float(critic_xi))
                    print("critic_mode:", "legacy_online" if critic_legacy_mode else "mixed_online_offline")
                print_index += 1
            ...
            if critic_legacy_mode:
                ...
                next_action_batch = _sample_hybrid_action_batch(next_state_batch, actor_new, old_policies, rho, action_dim)
                offline_state_batch, offline_action_batch, offline_costs_batch, offline_next_state_batch = _sample_offline_transition_batch(
                    offline_datasets,
                    grad_t,
                    state_dim,
                    action_dim,
                ...
                offline_next_action_batch = _sample_hybrid_action_batch(
                    offline_next_state_batch,
                    actor_new,
                    old_policies,
                    rho,
                    action_dim,
            ...
            critic.critic_update(
                ...
                gamma_reward,
                gamma_cost,
                offline_state_batch=offline_state_batch,
                offline_action_batch=offline_action_batch,
                offline_costs_batch=offline_costs_batch,
                offline_next_state_batch=offline_next_state_batch,
                offline_next_action_batch=offline_next_action_batch,
                critic_xi=critic_xi,
            )
            ...
            if q_update_index == q_update_time:
                ...
                q_hat_torch = critic.critic_value(
                    fused_state_torch,
                    fused_action_torch,
                    legacy_online_mode=critic_legacy_mode,
                )
                q_hat = _q_head_normalize(q_hat_torch.detach().cpu().numpy())
                q_hat_torch = torch.tensor(q_hat, dtype=torch.float, device=device)
                ...
                for head in range(constraint_dim + 1):
                    ...
                    actor_loss = _blend_online_offline_loss(actor_loss_online, actor_loss_offline, actor_xi, use_offline_data)
                    actor_loss.backward()
